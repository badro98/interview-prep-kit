    ...
    <aside className="flex w-72 shrink-0 flex-col border-r border-ink-700 bg-ink-800/50">
      <div className="border-b border-ink-700 p-4">
        <div className="flex items-center justify-between gap-2">
          ...
          <button
            onClick={handleNew}
            className="rounded-md bg-accent-500 px-2.5 py-1 text-[11px] font-semibold text-white transition hover:bg-accent-400"
          >
            + New
          </button>
        </div>
  ...
  return (
    ...
    >
      ...
      <span
        ...
        onClick={onDelete}
        title="Delete chat"
        className="shrink-0 rounded px-1 py-0.5 text-[10px] text-slate-600 opacity-0 transition group-hover:opacity-100 hover:bg-ink-600 hover:text-red-400"
      >
        ✕
      </span>
    </button>

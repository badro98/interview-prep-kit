    ...
    >
      <div className="flex max-h-[88vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-ink-600 bg-ink-800 shadow-2xl">
        <div className="flex items-center justify-between border-b border-ink-600 px-5 py-4">
          <div>
            <h3 className="text-base font-semibold text-white">Job settings</h3>
        ...
        <div className="flex-1 space-y-5 overflow-y-auto px-5 py-4">
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <input
                ...
                placeholder="Role"
                className="w-full rounded-md border border-ink-600 bg-ink-900 px-2.5 py-1.5 text-sm text-slate-200 focus:border-accent-500 focus:outline-none"
              />
            </div>
            <div>
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border border-ink-700 bg-ink-900/40 px-3 py-2.5">
            <p className="text-xs text-slate-400">
              Job description and notes live in the Context tab
            </p>
            <button
              onClick={() => onGoToContext?.()}
              className="shrink-0 rounded-md px-2.5 py-1 text-xs font-medium text-slate-300 hover:bg-ink-700"
            >
              Open Context
            </button>
          </div>
            ...
            <div>
              ...
              <div className="space-y-1.5">
                {profileEntries.map((entry) => (
                  <label
                    key={entry.id}
                    className="flex items-center gap-2 rounded-md border border-ink-700 bg-ink-900/40 px-3 py-2 text-sm text-slate-200 hover:bg-ink-900/70"
                  >
                    <input
                      type="checkbox"
                      checked={profileRefs.includes(entry.id)}
                      onChange={() => toggleProfileRef(entry.id)}
                      className="h-3.5 w-3.5 rounded border-ink-600 bg-ink-900 text-accent-500 focus:ring-accent-500"
                    />
                    {entry.name}
                  </label>
                ))}
